...
class TestAllAPIs(object):
    def test_all_apis(self, client):
        ...
        for i, client_id in enumerate(client_ids):
            ...
            response = client.patch('/clients/{}'.format(client_id), data=json.dumps(request_body), headers=headers)
            expect(response.status_code).to(equal(200))

        # Ensure that clients actually have roles and users
        for client_id in client_ids:
            response = client.get('/clients/{}'.format(client_id), headers=headers)
    ...
    def _cleanup(self, client, data_trust_id, role_ids=[], user_ids=[]):
        ...
        for user_id in user_ids:
            response = client.delete('/users/{}'.format(user_id), headers=headers)
            expect(response.status_code).to(equal(200))
